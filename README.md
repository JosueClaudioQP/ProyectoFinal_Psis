...
Actualmente el proyecto se encuentra organizado en módulos independientes, donde cada uno implementa una funcionalidad específica del sistema operativo Linux.

| Módulo | Estado |
|---------|:------:|
| Administrador de procesos | ✅ Implementado |
| Administrador de archivos | 🚧 En desarrollo |
| Comandos Linux | 🚧 En desarrollo |
| Sistema de respaldos | 🚧 En desarrollo |
| Analizador Bash | 🚧 En desarrollo |
| Cola de descargas | 🚧 En desarrollo |
...
Al ingresar al módulo se presenta un menú interactivo con las siguientes opciones:
...
4. Suspender proceso
5. Reanudar proceso
6. Mostrar árbol de procesos
0. Volver
```

Antes de mostrar el menú también se presenta información relevante del sistema como:
...
Permite visualizar todos los procesos activos del sistema ordenados por consumo de CPU.
...
Permite localizar procesos utilizando su nombre.

Antes de realizar la búsqueda se muestran los procesos con mayor consumo de CPU como referencia para el usuario.

Comando utilizado:

```bash
ps -C nombre_proceso -o pid,user,%cpu,%mem,comm
```
...
Permite terminar un proceso mediante su PID.

Antes de ejecutar la operación el sistema:

- verifica que el PID exista;
- solicita confirmación al usuario;
- ejecuta la finalización únicamente si la confirmación es positiva;
- registra automáticamente la acción en el historial.
...
Permite detener temporalmente la ejecución de un proceso sin finalizarlo.

Esta operación resulta útil cuando se desea pausar un proceso y reanudarlo posteriormente.
...
Permite continuar la ejecución de un proceso previamente suspendido.
...
Muestra gráficamente la jerarquía de procesos existente en el sistema operativo.
...
Todas las operaciones importantes realizadas por el usuario quedan registradas automáticamente.

Actualmente se almacenan las siguientes acciones:
...
```text
historial/procesos.log
```

Ejemplo de contenido:

```text
15/07/2026 20:34:15 - FINALIZAR PID 4582
15/07/2026 20:36:48 - SUSPENDER PID 2331
15/07/2026 20:38:12 - REANUDAR PID 2331
```
...
Para evitar errores durante la administración de procesos se incorporaron diversas validaciones.

Entre ellas:

- Verificación de existencia del PID.
- Confirmación antes de finalizar un proceso.
- Mensajes de error cuando la operación no puede ejecutarse.
- Registro automático de operaciones exitosas.

Estas validaciones permiten que el módulo sea más seguro y evitan operaciones accidentales sobre procesos inexistentes.
...
El archivo `procesos.c` contiene toda la lógica del módulo mientras que `procesos.h` define las funciones públicas utilizadas por el menú principal de la aplicación.

---

## Evidencias

Se recomienda incorporar capturas de pantalla durante las pruebas del módulo.
